...
function insertBranches($branches){
    ...
    for($i = 0; $i < count($branches); ++$i){
        $sql .= "('" . $branches[$i]['regnum'] . "', '" . mysqli_real_escape_string($link, $branches[$i]['city']) . "', '" . $branches[$i]['rid'] . "', '" . mysqli_real_escape_string($link, $branches[$i]['name']) . "', '" . mysqli_real_escape_string($link, $branches[$i]['adr']) . "', '" . $branches[$i]['longitude'] . "', '" . $branches[$i]['latitude'] . "', '" . $branches[$i]['phone'] . "', '" . $branches[$i]['phone_obn_val'] . "', '" . $branches[$i]['emails'] . "', '" . $branches[$i]['addinfo'] . "', '" . $branches[$i]['pay'] . "', '" . $branches[$i]['resource'] . "', '" . $branches[$i]['chek'] . "', '" . $branches[$i]['URL'] . "', '" . $branches[$i]['URL_city'] . "', '" . $branches[$i]['dop_inf'] . "', '" . $branches[$i]['latcity'] . "', '" . $branches[$i]['point'] . "', '" . $branches[$i]['google_oid'] . "', '" . $branches[$i]['google_site'] . "', '" . $branches[$i]['google_site_new'] . "', '" . $branches[$i]['google_full_name'] . "', '" . $branches[$i]['google_photo'] . "', '" . $branches[$i]['google_code'] . "', '" . $branches[$i]['google_source_url'] . "', '" . $branches[$i]['google_hash'] ."')" . (($i === count($branches) - 1) ? ';':', ');
    }
    $result = mysqli_query($link, $sql);
    if($result === false){
...
function insertAtms($atm){
    ...
    for($i = 0; $i < count($atm); ++$i){
        $sql .= "('" . $atm[$i]['regnum'] . "', '" . mysqli_real_escape_string($link, $atm[$i]['city']) . "', '" . $atm[$i]['rid'] . "', '" . mysqli_real_escape_string($link,$atm[$i]['name']) . "', '" . mysqli_real_escape_string($link,$atm[$i]['adr']) . "', '" . $atm[$i]['longitude'] . "', '" . $atm[$i]['latitude'] . "', '" . $atm[$i]['phone'] . "', '" . $atm[$i]['emails'] . "', '" . $atm[$i]['pay'] . "', '" . $atm[$i]['work_full_time'] . "', '" . $atm[$i]['org_pay'] . "', '" . $atm[$i]['send_pay'] . "', '" . $atm[$i]['print_pas'] . "', '" . $atm[$i]['addinfo'] . "', '" . $atm[$i]['resource'] . "', '" . $atm[$i]['chek'] . "', '" . $atm[$i]['URL'] . "', '" . $atm[$i]['URL_city'] . "', '" . $atm[$i]['dop_inf'] . "', '" . $atm[$i]['latcity'] . "', '" . $atm[$i]['point'] . "', '" . $atm[$i]['google_oid'] . "', '" . $atm[$i]['google_site'] . "', '" . $atm[$i]['google_site_new'] . "', '" . $atm[$i]['google_full_name'] . "', '" . $atm[$i]['google_photo'] . "', '" . $atm[$i]['google_code'] . "', '" . $atm[$i]['google_source_url'] . "', '" . $atm[$i]['google_hash'] . "', '" . $atm[$i]['nal_eur'] . "', '" . $atm[$i]['max_eur'] . "', '" . $atm[$i]['nal_usd'] . "', '" . $atm[$i]['max_usd'] . "')" . (($i === count($atm) - 1) ? ';': ', ');
    }
    $result = mysqli_query($link, $sql);
    if($result === false){
